...
class OverpassturboScrapper implements ScrapperInterface {
    ...
    public function scrap($cityId, $dataset, $input, $output)
    {
        ...
        $queries = [
            ...
            'racks' => '<osm-script output="json" timeout="25"><union><query type="node"><has-kv k="amenity" v="bicycle_parking"/><bbox-query {{bbox}}/></query><query type="way"><has-kv k="amenity" v="bicycle_parking"/><bbox-query {{bbox}}/></query><query type="relation"><has-kv k="amenity" v="bicycle_parking"/><bbox-query {{bbox}}/></query></union><print mode="body"/><recurse type="down"/><print mode="skeleton" order="quadtile"/></osm-script>',
            'cobblestone' => '<osm-script output="json" timeout="25"><union><query type="node"><has-kv k="surface" v="cobblestone"/><bbox-query {{bbox}}/></query><query type="way"><has-kv k="surface" v="cobblestone"/><bbox-query {{bbox}}/></query><query type="relation"><has-kv k="surface" v="cobblestone"/><bbox-query {{bbox}}/></query></union><print mode="body"/><recurse type="down"/><print mode="skeleton" order="quadtile"/></osm-script>',
        ];
